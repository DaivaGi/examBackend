...
import lt.techin.exam.api.dto.ErrorDto;
import lt.techin.exam.api.dto.ErrorFieldDto;
import lt.techin.exam.api.dto.mapper.ErrorFieldMapper;
import lt.techin.exam.exception.ExamServiceDisabledException;
...
//    }
    ...
    public ResponseEntity<ErrorDto> handleExamValidationException(HttpServletRequest request, ExamValidationException examValidationException) {
        logger.error("examValidationException: {}, for field: {}", examValidationException.getMessage(), examValidationException.getField());

        var errorStatus = HttpStatus.BAD_REQUEST;

        var errorFields = List.of(
                new ErrorFieldDto(examValidationException.getField(), examValidationException.getError(), examValidationException.getRejectedValue())
        );

        var errorDto = new ErrorDto(request.getRequestURL().toString(),
                errorFields,
                examValidationException.getMessage(),
                errorStatus.value(),
                errorStatus.getReasonPhrase(),
                request.getRequestURL().toString(),
                LocalDateTime.now());
    ...
    @ExceptionHandler(ExamServiceDisabledException.class)
    public ResponseEntity<Void> handleExamServiceDisabledException(HttpServletRequest request, ExamServiceDisabledException serviceDisabledException) {
        logger.error("ExamServiceDisabledException: {}", serviceDisabledException.getMessage());
